...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const voucherGuildId = String(body.voucher_guild_id || '').trim();
    ...
    if (voucherCode) {
      const { data: voucher, error: voucherError } = await db.from('organization_vouchers').select('*').eq('code', voucherCode).maybeSingle();
      ...
      const { data: redeemed, error: redeemError } = await db.from('organization_vouchers').update({ redeemed_at: new Date().toISOString(), redeemed_by_discord_id: String(discordUser.id), redeemed_organization_id: active.organization_id, organization_id: active.organization_id }).eq('id', voucher.id).is('redeemed_at', null).is('redeemed_organization_id', null).select('id').maybeSingle();
      if (redeemError) throw redeemError;
      if (!redeemed) return reply({ error: 'Voucherul a fost folosit între timp.' }, 409);
      const packageExpires = voucher.duration_days ? new Date(Date.now() + Number(voucher.duration_days) * 86400000).toISOString() : null;
      await db.from('app_settings').upsert({ organization_id: active.organization_id, key: 'organization_package', value: { code: voucher.package_code, unlimited: false, expires_at: packageExpires }, updated_at: new Date().toISOString() }, { onConflict: 'organization_id,key' });
      await db.from('organizations').update({ lifecycle_status: 'active', grace_until: null, active: true, updated_at: new Date().toISOString() }).eq('id', active.organization_id);
      await db.from('organization_lifecycle_events').insert({ organization_id: active.organization_id, event_type: 'voucher_redeemed', actor_discord_id: String(discordUser.id), details: { package_code: voucher.package_code, duration_days: voucher.duration_days } });
    }
