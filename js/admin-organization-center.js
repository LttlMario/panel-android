(() => {
  ...
  const ready = setInterval(() => {
    ...
    box.innerHTML = `<h2 class="text-lg font-black">Centru administrare organizații</h2><p class="mt-2 text-sm text-slate-400">Status, audit, roluri, webhook-uri și operațiuni rapide.</p>
      ...
      <div class="mt-4 grid gap-3 sm:grid-cols-2"><a class="rounded-xl bg-indigo-700 p-3 text-center font-bold" href="organizatii.html">Organizații</a><a class="rounded-xl bg-fuchsia-700 p-3 text-center font-bold" href="vouchere.html">Vouchere</a><a class="rounded-xl bg-cyan-700 p-3 text-center font-bold" href="diagnostic.html">Verificare sistem</a><a class="rounded-xl bg-amber-700 p-3 text-center font-bold" href="secrete-platforma.html">Secrete platformă</a><a class="rounded-xl bg-sky-700 p-3 text-center font-bold" href="setari-platforma.html">Setări platformă</a><a class="rounded-xl bg-emerald-700 p-3 text-center font-bold" href="administrare-organizatie.html">Status organizație</a></div>`;
    ...
    document.getElementById('org-admin-members').onclick=()=>{const table=document.querySelector('table');if(!table){alert('Încarcă mai întâi lista membrilor.');return;}const rows=[...table.querySelectorAll('tr')].map(row=>[...row.querySelectorAll('th,td')].map(cell=>`"${cell.innerText.replaceAll('"','""')}"`).join(','));const link=document.createElement('a');link.href=URL.createObjectURL(new Blob([rows.join('\n')],{type:'text/csv'}));link.download='membri.csv';link.click();URL.revokeObjectURL(link.href);};
